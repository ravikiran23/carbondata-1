...
public class TupleConversionAdapter {
  ...
  public Object[] getObjectArray(Result carbonTuple) {
    ...
    if (isNoDictionaryPresent) {
      ...
      for (int i = 0; i < noDicCount; i++) {
        noDicByteArr.add(carbonTuple.getKey().getNoDictionaryKeyByIndex(i));
      }
    ...
    row[index++] = carbonTuple.getKey().getDictionaryKey();
    return row;
  }
}
